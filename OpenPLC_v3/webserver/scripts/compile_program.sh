...
if [ "$OPENPLC_PLATFORM" = "win" ]; then
    ...
    echo "Generating object files..."
    g++ -std=gnu++17 -I ./lib -c Config0.c -lasiodnp3 -lasiopal -lopendnp3 -lopenpal -w
    if [ $? -ne 0 ]; then
        echo "Error compiling C files"
        echo "Compilation finished with errors!"
        echo "First section"
    ...
    g++ -std=gnu++17 -I ./lib -c Res0.c -lasiodnp3 -lasiopal -lopendnp3 -lopenpal -w
    if [ $? -ne 0 ]; then
        echo "Error compiling C files"
        echo "Compilation finished with errors!"
        echo "Second section"
    ...
    echo "Compiling main program..."
    if [ $# -eq 1 ]; then 
        g++ -std=gnu++17 *.cpp ../../../src/parseConfig.cpp ../../../src/shmSetup.cpp *.o -o openplc -I ./lib -I /usr/local/boost_1_79_0 -pthread -fpermissive `pkg-config --cflags --libs libmodbus` -lasiodnp3 -lasiopal -lopendnp3 -lopenpal -lrt -w 
    else 
        g++ -std=gnu++17 *.cpp *.o -o openplc -I ./lib -pthread -fpermissive `pkg-config --cflags --libs libmodbus` -lasiodnp3 -lasiopal -lopendnp3 -lopenpal -lrt -w
    fi
    #g++ -std=gnu++17 *.cpp ../../../src/parseConfig.cpp ../../../src/shmSetup.cpp *.o -o openplc -I ./lib -I /usr/local/boost_1_79_0 -pthread -fpermissive `pkg-config --cflags --libs libmodbus` -lasiodnp3 -lasiopal -lopendnp3 -lopenpal -lrt -w
    if [ $? -ne 0 ]; then
        echo "Error compiling C files"
