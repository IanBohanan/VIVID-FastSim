...
def hardware():
    if (flask_login.current_user.is_authenticated == False):
        ...
        if (flask.request.method == 'GET'):
            ...
            if (current_driver == "ml_linux"): return_str += "<option selected='selected' value='ml_linux'>MATLAB Enginge on Linux</option>"
            else: return_str += "<option value='ml_linux'>MATLAB Engine on Linux</option>"
            return_str += """
                        </select>
                        <br>
                        <br>
...
if __name__ == '__main__':
    ...
    if (conn != None):
        try:
            cur = conn.cursor()
            cur.execute("INSERT INTO Programs (Name, Description, File, Date_upload) VALUES (?, ?, ?, ?)", ("test", "test_program", "st_file.st", "1641401250"))
            conn.commit()
            cur.execute("SELECT * FROM Programs WHERE File=?", (st_file,))
